...
test('escaping handles safe characters without quotes', () => {
  const im = new IMCB()
  ...
  expect(im.parts('escape-shell')).toEqual(['-format', 'jpeg', '-format', 'test.jpg', '-format', '100%'])
})

test('escaping handles dangerous characters with quotes', () => {
  const im = new IMCB()

  im.comment('test with spaces')
  im.comment("test'with'quotes")
  im.comment('test with; semicolon')
  im.comment('test with | pipe')
  im.comment('test with $(command)')
  im.comment('test with `backticks`')

  const parts = im.parts('escape-shell')
  expect(parts[0]).toBe('-comment')
  expect(parts[1]).toBe("'test with spaces'")
  expect(parts[2]).toBe('-comment')
  expect(parts[3]).toBe("'test'\\''with'\\''quotes'")
  expect(parts[4]).toBe('-comment')
  expect(parts[5]).toBe("'test with; semicolon'")
  expect(parts[6]).toBe('-comment')
  expect(parts[7]).toBe("'test with | pipe'")
  expect(parts[8]).toBe('-comment')
  expect(parts[9]).toBe("'test with $(command)'")
  expect(parts[10]).toBe('-comment')
  expect(parts[11]).toBe("'test with `backticks`'")
})

test('escaping handles backslashes correctly', () => {
  const im = new IMCB()

  im.comment('test\\with\\backslashes')
  im.comment('test with \\ single backslash')

  const parts = im.parts('escape-shell')
  expect(parts[1]).toBe("'test\\\\with\\\\backslashes'")
  expect(parts[3]).toBe("'test with \\\\ single backslash'")
})

test('real live injections with draw', () => {
  const injectionPatterns = [
    '"; touch injections/inject-1; echo "',
    "'; touch injections/inject-2; echo '",
    '`touch injections/inject-3`',
    '$(touch injections/inject-4)',

    '; touch injections/inject-5',
    '&& touch injections/inject-6',
    '| touch injections/inject-7',
    '|| touch injections/inject-8',

    '\n touch injections/inject-9',
    '\r touch injections/inject-10',

    '\\" ; touch injections/inject-11 ; echo \\"',
    "\\' ; touch injections/inject-12 ; echo \\'",

    '"; }; touch injections/inject-13; function x() { echo "',
    '\'; exec("touch injections/inject-14"); echo \'',

    '"; touch injections/inject-15 #',
    '\u0022; touch injections/inject-16; echo \u0022',

    '\\"; touch injections/inject-17; echo "',
    "\\'; touch injections/inject-18; echo '",
    '\\\\"; touch injections/inject-18; echo \'',
    "\\\\'; touch injections/inject-19; echo '",
  ]

  injectionPatterns.forEach(pattern => {
    try {
      const im = new IMCB().size(200, 200).xc('white')
      im.draw(draw => draw.text(10, 10, pattern)).command('info:')

      const commandParts = ['magick', ...im.parts('escape-shell')]
  ...
  const injectionFiles = readdirSync('injections').filter(f => f.startsWith('inject-'))
